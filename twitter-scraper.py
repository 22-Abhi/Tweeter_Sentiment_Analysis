...
def main(argv):
    ...
    with open('tweetreply.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        print("hari")
        writer.writerow(["Count", "Repliedbyusername", "Repliedbyuser", "Repliedto", "Reply","Date"] )
        file.close()
        ...
        if tweet_url is not None:
            ...
            for reply in get_replies(tweet):
                ...
                if not short_output: 
                    ...
                    print('%s\\----------------------------------' % (tabs))
                    print ('')
                    with open('tweetreply.csv', 'a', newline='') as file:
                        writer = csv.writer(file)
                        writer.writerow([count,j['user']['name'] ,j['user']['screen_name'],tabs,j['text'],human_date ])
                        file.close()
